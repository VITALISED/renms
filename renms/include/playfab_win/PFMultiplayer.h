...
#pragma pack(push, 8)
    ...
    /// remain valid until the title's corresponding <see cref="PFMultiplayerFreeMemoryCallback" /> is invoked to
    /// release it. If this is not possible, the callback must return nullptr to fail the allocation. Memory allocation
    /// failures are sometimes considered benign but will usually cause current Multiplayer library operation(s) to
    /// fail.
    /// </para>
    /// <para>
    ...
    /// An opaque identifier representing the Multiplayer library internal category of memory being allocated.
    /// Generally, this value should be ignored but can be used to assist in diagnosing and debugging Multiplayer memory
    /// allocations.
    /// </param>
    /// <returns>
    ...
    typedef _Ret_maybenull_ _Post_writable_byte_size_(size) void *(__stdcall *PFMultiplayerAllocateMemoryCallback)(
        size_t size, uint32_t memoryTypeId);

    /// <summary>
    /// A callback invoked every time a previously allocated memory buffer is no longer needed by the Multiplayer
    /// library and can be freed.
    /// </summary>
    /// <remarks>
    /// This callback is optionally installed using the <see cref="PFMultiplayerSetMemoryCallbacks()" /> method.
    /// <para>
    /// The callback is invoked whenever the Multiplayer library has finished using a memory buffer previously returned
    /// by the title's corresponding <see cref="PFMultiplayerAllocateMemoryCallback" />, so that the title can free the
    /// memory buffer.
    /// </para>
    /// </remarks>
    /// <param name="pointer">
    ...
    /// <seealso cref="PFMultiplayerSetMemoryCallbacks" />
    typedef void(__stdcall *PFMultiplayerFreeMemoryCallback)(_Post_invalid_ void *pointer, uint32_t memoryTypeId);
...
#pragma pack(pop)
    ...
    _Null_terminated_ const char *PFMULTIPLAYER_API PFMultiplayerGetErrorMessage(HRESULT error) noexcept;
    ...
    /// the Multiplayer library for new memory buffers instead of using its default allocation functions. <para> The
    /// <paramref name="allocateMemoryCallback" /> and <paramref name="freeMemoryCallback" /> parameters must both be
    /// non-null.
    /// </para>
    /// <para>
    ...
    PFMultiplayerSetMemoryCallbacks(
        ...
        _In_ PFMultiplayerFreeMemoryCallback freeMemoryCallback) noexcept;
    ...
    /// given type.
    /// <para>
    /// This method may be called at any time before or after <see cref="PFMultiplayerInitialize()" /> and will take
    /// effect immediately. Thread processor settings are persisted across calls to <see
    /// cref="PFMultiplayerUninitialize()" /> and PFMultiplayerInitialize(). When there are more than 64 cores present,
    /// this method always applies to processor group 0.
    /// </para>
    /// <para>
    /// In order to specify any processor, pass <c>PFMultiplayerAnyProcessor</c> as the
    /// <paramref name="threadAffinityMask" /> parameter. This is also the default value the Multiplayer library will
    /// use if this method is never called.
    /// </para>
    /// </remarks>
    /// <param name="threadId">
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMultiplayerSetThreadAffinityMask(PFMultiplayerThreadId threadId, uint64_t threadAffinityMask) noexcept;
    ...
    /// </para>
    /// <para>
    /// The provided <paramref name="playFabTitleId" /> must be the same PlayFab Title ID used to acquire the PlayFab
    /// Entity Keys and Entity Tokens that will be passed to <see cref="PFMultiplayerSetEntityToken()" />.
    /// </para>
    /// </remarks>
    ...
    PFMultiplayerInitialize(
        _Null_terminated_ const char *playFabTitleId, _Outptr_ PFMultiplayerHandle *handle) noexcept;
    ...
    /// connectivity to these users has been lost), so best practice is to call <c>PFLobbyLeave</c> on all lobbies and
    /// wait for the corresponding <c>PFLobbyLeaveLobbyCompletedStateChange</c> to have the local users exit any
    /// existing lobbies. <para> This method is not thread-safe and may not be called concurrently with other
    /// Multiplayer library methods. After calling this method, all Multiplayer library state is invalidated.
    /// </para>
    /// <para>
    /// Every call to <see cref="PFMultiplayerInitialize()" /> should have a corresponding PFMultiplayerUninitialize()
    /// call.
    /// </para>
    /// </remarks>
    /// <param name="handle">
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMultiplayerUninitialize(_Post_invalid_ PFMultiplayerHandle handle) noexcept;

    /// <summary>
    /// Sets the token that should be used for authentication when performing library actions on behalf of an entity. If
    /// a token has previously been set for the entity, this replaces its previous token.
    /// </summary>
    /// <remarks>
    /// This method takes a PlayFab Entity Key as <paramref name="entity" /> and a PlayFab Entity Token as
    /// <paramref name="token" />. When the library performs operations on behalf of an entity that require
    /// authentication or authorization, such as creating or updating a lobby, the library will look up a token
    /// associated with the entity to use for the operation. If no token has previously been set for the entity, the
    /// operation will synchronously fail. During the asynchronous operation, the PlayFab service will validate that the
    /// token is valid, is not expired, is associated with the Entity ID provided, and is authorized to perform the
    /// operation. If these conditions aren't met, the operation will fail. <para> A PlayFab Entity Key and Entity Token
    /// can be obtained from the output of a PlayFab login operation and then provided as input to this method.
    /// </para>
    /// <para>
    /// The provided <paramref name="entity" /> and <paramref name="token" /> must have been acquired using the same
    /// PlayFab Title ID that was passed to <see cref="PFMultiplayerInitialize()" />.
    /// </para>
    /// <para>
    /// The Multiplayer library makes a copy of the supplied PlayFab Entity Token for use in subsequent operations that
    /// require authentication or authorization of the local user, such as <c>PFMultiplayerCreateAndJoinLobby</c>. If
    /// the token provided to this call is expired or otherwise invalid, operations that require a valid token will
    /// fail. A new, valid token can be provided to the Multiplayer library by calling this method again using the same
    /// entity key.
    /// </para>
    /// <para>
    ...
    /// new token when the previously supplied token is halfway through its validity period. On platforms that may enter
    /// a low power state or otherwise cause the application to pause execution for a long time, preventing the token
    /// from being refreshed before it expires, the token should be checked for expiration once execution resumes.
    /// </para>
    /// <para>
    ...
    /// </param>
    /// <returns>
    /// <c>S_OK</c> if creating the local user succeeded or an error code otherwise. The human-readable form of the
    ...
    PFMultiplayerSetEntityToken(
        PFMultiplayerHandle handle, const PFEntityKey *entity, _Null_terminated_ const char *entityToken) noexcept;
