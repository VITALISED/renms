...
#pragma pack(push, 8)
    ...
    enum class PFMatchmakingTicketStatus : uint32_t
    {
        ...
        /// <summary>
        /// The matchmaking ticket is waiting for all remote users specified in the <c>membersToMatchWith</c> field of
        /// its configuration to join the ticket via <see cref="PFMultiplayerJoinMatchmakingTicketFromId" />.
        /// </summary>
        WaitingForPlayers = 2,
    ...
    /// <see cref="PFMultiplayerStartProcessingMatchmakingStateChanges()" /> for the title to handle and then promptly
    /// pass back via the <see cref="PFMultiplayerFinishProcessingMatchmakingStateChanges()" /> method. <para> The
    /// <c>stateChangeType</c> field indicates which kind of state change occurred, and this base structure should then
    ...
    struct PFMatchmakingTicketConfiguration
    {
        ...
        /// represents a group of friends or fireteam that are looking for a match together. A match can't be found
        /// until all specified users join the ticket via <see cref="PFMultiplayerJoinMatchmakingTicketFromId" />.
        /// </remarks>
    ...
    struct PFMatchmakingMatchDetails
    {
        ...
        /// </summary>
        /// <remarks>
        /// This field will be populated if the matchmaking queue associated with the ticket has enabled PlayFab
    ...
    struct PFMatchmakingServerBackfillTicketConfiguration
    {
        ...
        /// clients that are matched with this ticket. Clients will be able to use this information to the connect to
        /// the game server. <para> The server details are optional. If no server details need to be provided to clients
        /// on match completion, this value can be nullptr. Similarly, any of the fields in the server details don't
...
#pragma pack(pop)
    ...
    /// services, and retrieves a list of all changes currently available for the title since the last call to this
    /// method. The title should use the provided array of 0 or more changes to update its own state or UI, and then
    /// call <see cref="PFMultiplayerFinishProcessingMatchmakingStateChanges()" /> with them in a timely manner. <para>
    /// Matchmaking library state exposed by the library can change during this call, so you must be thread-safe in your
    /// use of it. PFMultiplayerStartProcessingMatchmakingStateChanges() should be called frequently-- at least once per
    ...
    /// before calling PFMultiplayerFinishProcessingMatchmakingStateChanges().
    /// </para>
    /// <para>
    ...
    /// to <see cref="PFMultiplayerFinishProcessingMatchmakingStateChanges()" />.
    /// </param>
    /// <returns>
    ...
    PFMultiplayerStartProcessingMatchmakingStateChanges(
        ...
        _Outptr_result_buffer_(*stateChangeCount) const PFMatchmakingStateChange *const **stateChanges) noexcept;
    ...
    /// <see cref="PFMultiplayerStartProcessingMatchmakingStateChanges()" /> have now been handled by the title, so
    /// their associated resources can be reclaimed. You may call PFMultiplayerFinishProcessingMatchmakingStateChanges()
    ...
    /// PFMultiplayerFinishProcessingMatchmakingStateChanges() exactly once, but may be returned out of order and may be
    /// interleaved with state changes from other calls to PFMultiplayerStartProcessingMatchmakingStateChanges(). <para>
    ...
    /// by <see cref="PFMultiplayerStartProcessingMatchmakingStateChanges()" /> that have now been handled by the title.
    /// </param>
    /// <param name="stateChanges">
    ...
    PFMultiplayerFinishProcessingMatchmakingStateChanges(
        ...
        _In_reads_(stateChangeCount) const PFMatchmakingStateChange *const *stateChanges) noexcept;

    /// <summary>
    /// Creates a matchmaking ticket for one or more local users.
    /// </summary>
    /// <remarks>
    /// The library automatically, and asynchronously, will submit all local users on a ticket to the matchmaking
    /// service. Each time the ticket status changes, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will
    /// be provided. The ticket status can be queried at any time via <see cref="PFMatchmakingTicketGetStatus()" />. The
    /// ticket immediately starts in the <c>PFMatchmakingTicketStatus::Creating</c> state. <para> When the ticket has
    /// completed, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will be provided. At that point, a match
    /// will have been found or the ticket stopped due to failure. On success, the match that was found can be queried
    /// via <see cref="PFMatchmakingTicketGetMatch()" />.
    /// </para>
    /// <para>
    ...
    /// The array of local user attribute strings. There should be one attribute string for each local user. Each
    /// attribute string should either be an empty string or a serialized JSON object. For example,
    ...
    PFMultiplayerCreateMatchmakingTicket(
        PFMultiplayerHandle handle, uint32_t localUserCount, _In_reads_(localUserCount) const PFEntityKey *localUsers,
        _In_reads_(localUserCount) const char *const *localUserAttributes,
        const PFMatchmakingTicketConfiguration *configuration, _In_opt_ void *asyncContext,
        _Outptr_ PFMatchmakingTicketHandle *ticket) noexcept;
    ...
    /// service. Each time the ticket status changes, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will
    /// be provided. The ticket status can be queried at any time via <see cref="PFMatchmakingTicketGetStatus()" />. The
    /// ticket immediately starts in the <c>PFMatchmakingTicketStatus::Joining</c> state. <para> When the ticket has
    /// completed, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will be provided. At that point, a match
    /// will have been found or the ticket stopped due to failure. On success, the match that was found can be queried
    /// via <see cref="PFMatchmakingTicketGetMatch()" />.
    /// </para>
    ...
    PFMultiplayerJoinMatchmakingTicketFromId(
        PFMultiplayerHandle handle, uint32_t localUserCount, _In_reads_(localUserCount) const PFEntityKey *localUsers,
        _In_reads_(localUserCount) const char *const *localUserAttributes, _Null_terminated_ const char *ticketId,
        _Null_terminated_ const char *queueName, _In_opt_ void *asyncContext,
        _Outptr_ PFMatchmakingTicketHandle *ticket) noexcept;
    ...
    /// </summary>
    /// <remarks>
    /// This method destroys the matchmaking ticket and reclaims local resources associated with it. If the ticket has
    /// not yet completed, a background operation will perform a single best-effort attempt to cancel the ticket with
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMultiplayerDestroyMatchmakingTicket(PFMultiplayerHandle handle, PFMatchmakingTicketHandle ticket) noexcept;
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMatchmakingTicketGetStatus(PFMatchmakingTicketHandle ticket, _Out_ PFMatchmakingTicketStatus *status) noexcept;
    ...
    /// be processed, or the cancelation request may fail due to networking or service errors. If the cancelation
    /// attempt fails but is retriable, the library will continue to retry the cancelation. Otherwise, a <see
    /// cref="PFMatchmakingTicketCompletedStateChange" /> will be provided that indicates the ticket failed.
    /// </para>
    /// </remarks>
    /// <param name="ticket">
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMatchmakingTicketCancel(PFMatchmakingTicketHandle ticket) noexcept;
    ...
    HRESULT
    PFMULTIPLAYER_API
    PFMatchmakingTicketGetTicketId(PFMatchmakingTicketHandle ticket, _Outptr_ const char **id) noexcept;

    /// <summary>
    /// Provides the match, if one has been found.
    /// </summary>
    /// <remarks>
    /// This method will fail if the ticket isn't in the <c>PFMatchmakingTicketStatus::Matched</c> state. The ticket
    /// state can be retrieved via <see cref="PFMatchmakingTicketGetStatus" />.
    /// </remarks>
    /// <param name="ticket">
    /// The handle of the matchmaking ticket.
    /// </param>
    ...
    PFMatchmakingTicketGetMatch(
        PFMatchmakingTicketHandle ticket, _Outptr_ const PFMatchmakingMatchDetails **match) noexcept;
    ...
    /// To use this feature, you must define PFMULTIPLAYER_INCLUDE_SERVER_APIS before including PFMatchmaking.h.
    /// <para>
    /// The library automatically, and asynchronously, will submit a server backfill ticket to the matchmaking service.
    /// Each time the ticket status changes, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will be
    /// provided. The ticket status can be queried at any time via <see cref="PFMatchmakingTicketGetStatus()" />. The
    /// ticket immediately starts in the <c>PFMatchmakingTicketStatus::Creating</c> state.
    /// </para>
    /// <para>
    /// When the ticket has completed, a <see cref="PFMatchmakingTicketStatusChangedStateChange" /> will be provided. At
    /// that point, a match for the backfill ticket will have been found or the ticket stopped due to failure. On
    /// success, the match that was found can be queried via <see cref="PFMatchmakingTicketGetMatch()" />. The
    /// <c>regionPreferences</c> in <see cref="PFMatchmakingMatchDetails" /> will only contain the region that the
    ...
    PFMultiplayerCreateServerBackfillTicket(
        PFMultiplayerHandle handle, const PFEntityKey *server,
        const PFMatchmakingServerBackfillTicketConfiguration *configuration, _In_opt_ void *asyncContext,
        _Outptr_ PFMatchmakingTicketHandle *ticket) noexcept;
#endif // PFMULTIPLAYER_INCLUDE_SERVER_APIS
